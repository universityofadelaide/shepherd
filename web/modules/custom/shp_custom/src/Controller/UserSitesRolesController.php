...
class UserSitesRolesController extends ControllerBase {
  ...
  public function index($account_id) {
    ...
    $user_role_ids = \Drupal::entityQuery('paragraph')
      ->condition('type', 'shp_user_role')
      ->condition('field_shp_user_id', $account_id)
      ->execute();
    ...
    $site_ids = \Drupal::entityQuery('node')
      ...
      ->condition('field_shp_users', $user_role_ids, 'IN')
      ->execute();
    ...
    foreach ($sites as $site) {
      foreach ($site->field_shp_users as $user_role) {
        if ($account_id == $user_roles[$user_role->target_id]->field_shp_user_id->value) {
          $rows[] = [
            $site->title->value,
            $options[$user_roles[$user_role->target_id]->field_shp_role->value],
          ];
        }
      }
    }
