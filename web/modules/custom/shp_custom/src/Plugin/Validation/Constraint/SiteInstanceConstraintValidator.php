...
/**
 * @file
 * Contains \Drupal\shp_custom\Plugin\Validation\Constraint\SiteInstanceConstraintValidator.
 */
...
class SiteInstanceConstraintValidator extends ConstraintValidator {
  ...
  protected function validateNodeEntity(NodeInterface $node, SiteInstanceConstraint $constraint) {
    ...
    foreach ($node->field_shp_platform->entity->field_shp_web_servers->getValue() as $web_server) {
      $instances = \Drupal::entityQuery('node')
        ->condition('type', 'shp_site_instance')
        ->condition('field_shp_server', $web_server['target_id'])
        ->execute();
   ...
   *   Server entity.
   * @param \Drupal\shp_custom\Plugin\Validation\Constraint\SiteInstanceConstraint $constraint
