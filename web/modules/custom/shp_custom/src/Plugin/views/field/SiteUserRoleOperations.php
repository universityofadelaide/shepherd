...
class SiteUserRoleOperations extends FieldPluginBase {
  ...
  public function render(ResultRow $values) {
    ...
    $uid = $values->users_field_data_paragraph__field_shp_user_uid;
    $build = [];
    if ($uid) {
      $edit_url = Url::fromRoute('shp_custom.user-edit-form', ['node' => $node_id, 'user' => $uid]);
      $delete_url = Url::fromRoute('shp_custom.user-delete-form', ['node' => $node_id, 'user' => $uid]);
