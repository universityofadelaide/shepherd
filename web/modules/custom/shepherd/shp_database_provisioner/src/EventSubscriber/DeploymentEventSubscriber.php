  ...
  public function databasePopulate(OrchestrationEnvironmentEvent $event) {
    ...
    if (!empty($project->field_shp_default_sql->target_id)) {
      ...
      $orchestration_provider->executeJob(
        ...
        "wget $public_filename -O /tmp/dump.sql; drush -r web sqlq --file=/tmp/dump.sql; drush -r web updb --entity-updates -y; robo config:import-plus; drush -r web cr; rm /tmp/dump.sql"
      );
    }
  }
