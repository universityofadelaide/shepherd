...
$example_repository = getenv("DRUPAL_EXAMPLE_REPOSITORY") ?:
  'git@gitlab.adelaide.edu.au:web-team/ua-wcms-foundation.git';
...
if (empty($token)) {
  ...
  echo "You can then safely re-run `robo dev:wcms-content-generate`\n";
  exit(1);
}
...
$nodes = $stg->loadByProperties(['title' => 'WCMS project']);
...
if (!$project = reset($nodes)) {
  $project = Node::create([
    ...
    'field_shp_git_default_ref' => 'develop',
    'field_shp_builder_image' => [['value' => 'uofa/s2i-shepherd-drupal:composer2']],
    'field_shp_build_secret' => [['value' => 'build-key']],
    'field_shp_env_vars' => [
      ['key' => 'SHEPHERD_INSTALL_PROFILE', 'value' => 'ua'],
      ['key' => 'MEMCACHE_ENABLED', 'value' => '1'],
...
$nodes = $stg->loadByProperties(['title' => 'WCMS Site']);
...
if (!$env = reset($nodes)) {
  $env = Node::create([
    ...
    'field_shp_domain'           => 'uawcms-0.' . $domain_name,
